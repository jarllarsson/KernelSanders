#ifndef RAYTRACE_SETUP_H
#define RAYTRACE_SETUP_H

// =======================================================================================
//                                      RaytraceSetup
// =======================================================================================

#define MAXSPHERES 2
#define MAXPLANES 1
#define MAXTRIS 100
#define MAXBOXES 2
#define MAXLIGHTS 1
#define MAXMESHLOCAL_VERTSBIN 2000
//#define MAXMESHLOCAL_INDICESBIN MAXMESHLOCAL_VERTSBIN*3 // estimate
#define MAXMESHLOCAL_INDICESBIN MAXMESHLOCAL_VERTSBIN // estimate

#define SPACEDIST 200.0
#define SPHERERADIUS 5.0

#define PI 3.141592653589793238462643383279502884197169399375105820
#define TWOPI 2.0*PI
#define TORAD PI/180
#define TODEG 180/PI
#define PIOVER180 TORAD

#define MAX_INTERSECT_DIST 9999999.0f

#endif